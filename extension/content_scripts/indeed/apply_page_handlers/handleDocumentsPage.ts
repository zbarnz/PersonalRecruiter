export async function handleDocumentsPage(
  coverLetter: {
    buffer: Buffer | null;
    text: string | null;
  } | null
  ...
  if (fileInputElement instanceof HTMLElement) {
    // This check ensures fileInputElement is an HTMLElement
  ...
  // and try to populate the cover letter text input instead
    ...
    const coverLetterButton = document.querySelector(
      "#write-cover-letter-selection-card"
    );
    if (coverLetterButton instanceof HTMLElement) {
      // Ensure coverLetterButton is an HTMLElement
